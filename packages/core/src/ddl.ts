...
export function columnRef(table: string, column: string): string {
  return `${qualified(table)}.${quoteIdent(column)}`
}

/**
 * `COMMENT ON <target> IS ...`, where target is `TABLE public.users` or
 * `COLUMN public.users.email`. A removed comment is `NULL`, not an empty string.
 */
export function commentStatement(target: string, comment: string | null): string {
  return `COMMENT ON ${target} IS ${comment === null ? 'NULL' : quoteLiteral(comment)}`
}
...
export function createTableStatements(table: Table): string[] {
  ...
  const statements = [`CREATE TABLE ${qualified(key)} (\n  ${body.join(',\n  ')}\n)`]

  if (table.comment) {
    statements.push(commentStatement(`TABLE ${qualified(key)}`, table.comment))
  }
  for (const name of table.columnOrder) {
    const column = table.columns[name]
    if (!column.comment) continue
    statements.push(commentStatement(`COLUMN ${columnRef(key, name)}`, column.comment))
  }
  return statements
}
