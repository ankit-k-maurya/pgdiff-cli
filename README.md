...
`@pgdiff/core` has two entry points. The default one is the planner — snapshot
types, the diff engine, the ordering rules and the SQL writer — and it imports
nothing outside TypeScript, so the browser bundles it directly. Reading a
snapshot from a live server needs `pg`, so it sits behind its own subpath:

```ts
import { createMigration, phaseLabel } from '@pgdiff/core'
import { introspect, listSchemas } from '@pgdiff/core/introspect'
```

Inside the core, `src/diff/` holds one module per kind of object it compares
(schemas, enums, tables, columns, constraints, indexes); `src/plan.ts` orders
what they produce and renders the SQL.
